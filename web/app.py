...
st.markdown("""<style>
    ...
    .stDeployButton, .stActionButton {display: none;}
    .history-item { padding: 0.3rem 0.5rem; border-radius: 4px; margin: 0.1rem 0;
        cursor: pointer; font-size: 0.82rem; border-bottom: 1px solid #eee; }
...
def main():
    ...
    # ================================================================
    # LEFT SIDEBAR: Chat History + Settings
    # ================================================================
    with st.sidebar:
        st.markdown("### 💬 对话记录")
        ...
        # Group messages by conversation (user Q + assistant A pairs)
        ...
        while i < len(msgs):
            if msgs[i]["role"] == "user":
                ...
                t = msgs[i].get("time", "")
                groups.append({"q": q, "a": a, "t": t, "idx": i})
                i += 2
            else:
                i += 1

        # Show recent 20 groups
        for g in reversed(groups[-20:]):
            q_short = g["q"][:30] + ("..." if len(g["q"]) > 30 else "")
            st.markdown(
                f'<div class="history-item">'
                f'<b>{q_short}</b><br>'
                f'<span class="history-date">{g["t"]}</span>'
                f'</div>',
        ...
        # Settings
        st.markdown("### ⚙️")
        top_k = st.slider("搜索范围", 1, 20, 5, label_visibility="collapsed")

    # ================================================================
    # MAIN AREA: Chat + Upload
    # ================================================================
    col_main, col_upload = st.columns([5, 1])
    ...
    with col_upload:
        with st.popover("📎 上传", use_container_width=True):
            ...
            try:
                ...
                st.caption(f"知识库: {stats.get('total_chunks', 0)} 条")
            except Exception:
                pass
